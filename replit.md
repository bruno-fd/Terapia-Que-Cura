...
- Required env for billing: secret `ASAAS_API_KEY` (sent as the `access_token` header; this is a single global secret, so swap it to a production key when going live). `ASAAS_BASE_URL` defaults to the sandbox (`https://sandbox.asaas.com/api/v3`); the development env var pins the sandbox and a production env var pins `https://api.asaas.com/v3`. Optional `ASAAS_WEBHOOK_TOKEN` enables `asaas-access-token` header verification on the webhook. Optional `ASAAS_WEBHOOK_EMAIL` sets the webhook failure-notification email (falls back to the Asaas account email).
- The Asaas webhook is registered programmatically on server startup (`artifacts/api-server/src/lib/registerAsaasWebhook.ts`, called from `index.ts` after `listen`). It is idempotent (matched by URL `https://<REPLIT_DOMAINS first host>/api/assinatura/webhook`), subscribes to PAYMENT_CONFIRMED/RECEIVED/OVERDUE/DELETED/REFUNDED, and self-heals (re-enables / fixes events). This makes payment -> `ativa` -> visible on `/advogados` automatic, with no manual Asaas dashboard setup. Registration is skipped (logged, non-fatal) when `ASAAS_API_KEY` or a public domain is missing; any failure is logged and never crashes startup.
